...
class Fav extends Component {
  ...
  checkFav = () => {
    let index = this.props.favourites?.findIndex(
      item => item?.fID === this.props.item?._id,
    );
  ...
  render() {
    return (
      <TouchableOpacity
        onPress={() => {
          this.state.fav ? this.handleFavRemove() : this.handleFavAdd();
        }}>
        {this.state.loading ? (
          ...
          <FontAwesome
            size={30}
            name={'heart'}
            color={this.state.fav ? colors.orange : colors.greyText}
          />
        )}
      </TouchableOpacity>
    );
