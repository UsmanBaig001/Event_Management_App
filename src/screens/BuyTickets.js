                        ...
                        this.state.allEvents?.length > 0 ?
                            <FlatList
                                ...
                                renderItem={({ item, index }) =>
                                    <TicketCard onPress={() => this.onChange(item)} item={item?.eventID ? { ...item?.eventID, reviews: item?.reviews, averageRating: item?.averageRating } : null} {...{ index }} key={index} />
                                }
                            />
                            :
                            <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }} >
                                <LatoText text={"Tickets list empty."} fontSize={18} color={colors.orange} />
                            </View>
                    }
                </View>
