...
class Login extends Component {
  ...
  login = async () => {
    ...
    if (!email) alert(LAN.EN.LOGIN.ALERTS.VALIDATIONS.INVALID_EMAIL);
    else if (!password) alert(LAN.EN.LOGIN.ALERTS.VALIDATIONS.INVALID_PASSWORD);
    else {
          ...
          <View
            ...
            <View style={{alignItems: 'center'}}>
              <Input
                ...
                source={EMAIL_ICON}
                placeholder={LAN.EN.LOGIN.FIELDS.EMAIL}
                marginTop={20}
              />
              <Input
                ...
                placeholder={LAN.EN.LOGIN.FIELDS.PASSWORD}
                marginTop={15}
              />
              <View
                style={{
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  <TouchableOpacity
                    ...
                    {this.state.remenber && (
                      <Entypo name="check" size={10} color={colors.orange} />
                    )}
                  </TouchableOpacity>
                  <LatoText
                    text={LAN.EN.LOGIN.REMEMBER}
                    fontSize={12}
                    marginLeft={10}
                  />
                </View>
            ...
            <View
              ...
              <Button
                onPress={() => this.login()}
                width="90%"
                text={LAN.EN.LOGIN.SIGN_IN}
              />
            </View>
          </View>

          <Text style={{textAlign: 'center', marginTop: 50, marginBottom: 20}}>
            <LatoText text={LAN.EN.LOGIN.SUB_TITLE} color={colors.white} />
            <LatoText
              onPress={() => this.props.navigation.navigate('Signup')}
              text={LAN.EN.LOGIN.SIGN_UP}
              color={colors.orange}
              fontSize={16}
