...
const VerifyEmail = (props) => {
    ...
    const resendCode = async () => {
        setLoading(true)
        let res = await props._sendEmailOtp(props.route.params.requestBody.email, "signup")
        setLoading(false)
        if (res.success) {
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : null} style={{ paddingTop: sHeight }} >
            <Image style={{ position: "absolute", width: "100%", height: hp(100), zIndex: -1, top: 0 }} source={SPLASH_BG} />
            <ScrollView contentContainerStyle={{ width: "100%", minHeight: hp(100), }} >

                <View style={{ width: "85%", height: 380, backgroundColor: colors.white, borderRadius: 20, alignSelf: "center", marginTop: 100 }} >
                    ...
                    <LatoText fontSize={15} alignSelf={"center"} text={"Verify OTP"} textAlign="center" marginTop={30} />
                    <LatoText fontSize={12} color={colors.greyText} alignSelf={"center"} width={"80%"} text={"Please check email inbox for OTP"} textAlign="center" marginTop={20} />
                ...
                <Text style={{ textAlign: "center", marginTop: 50, marginBottom: 20 }} >
                    ...
                    <LatoText onPress={() => resendCode()} text={"Resend?"} color={colors.orange} fontSize={16} fontName="Poppins-Bold" />
                </Text>
            </ScrollView>
