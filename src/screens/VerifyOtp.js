...
const VerifyOtp = (props) => {
    ...
    const resendCode = async () => {
        setLoading(true)
        let res = await props._sendEmailOtp(props.route.params.requestBody?.email, "forget")
        setLoading(false)
        if (res.success) {
    ...
    const verifyCode = async () => {
        if (value.length === 5 && code === value) {
            ...
            setValue("")
            props.navigation.navigate("ResetPassword", { email: props.route.params.requestBody?.email })
        } else {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : null} style={{ paddingTop: sHeight }} >
            ...
            <ScrollView contentContainerStyle={{ width: "100%", minHeight: hp(100), }} >
                ...
                <View style={{ width: "85%", height: 380, backgroundColor: colors.white, borderRadius: 20, alignSelf: "center", marginTop: 100 }} >
                    ...
                    <LatoText fontSize={12} color={colors.greyText} alignSelf={"center"} width={"80%"} text={"Please check inbox for OTP"} textAlign="center" marginTop={20} />
